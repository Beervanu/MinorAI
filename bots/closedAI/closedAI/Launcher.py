...
class LauncherBot(Turret):
	def __init__(self, ct: Controller):
		...
		self.our_core_pos: Position = None
		self.find_core_pos(ct)
		if self.our_core_pos is None:
			eprint('No core pos')


	def find_core_pos(self,ct:Controller):
	...
	def pick_throw_target(self, ct: Controller, bot_pos):
		...
		positions = ct.get_nearby_tiles()

		filtered_pos = sorted(filter(lambda pos: ct.is_tile_passable(pos), positions), key=lambda pos: self.chebyshev(pos, self.our_core_pos), reverse=True)
		if filtered_pos:
			print('tried to launch')
			return filtered_pos[0]
		return None
		# if self.our_core_pos is None:
		# 	return None
		...
		# 	if not self.is_valid_position(target) or not ct.is_in_vision(target):
		# 		continue
